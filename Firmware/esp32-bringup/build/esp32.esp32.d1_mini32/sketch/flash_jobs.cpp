#line 1 "/media/nvme_raid0/mPC/cyberdeck-scout/Firmware/esp32-bringup/flash_jobs.cpp"
...
static void task_attiny(void *) {
  ...
  Serial.println("[FLASH] ATtiny flash job start");

  if (!LittleFS.exists(PATH_ATTINY)) {
    Serial.printf("[FLASH] missing staged file %s\n", PATH_ATTINY);
    set_err("No staged file; upload first");
    g_last_ok = false;
    g_busy = false;
    set_phase("idle");
  ...
  if (!f) {
    Serial.printf("[FLASH] failed to open staged file %s\n", PATH_ATTINY);
    set_err("Open staged file failed");
    g_last_ok = false;
    g_busy = false;
    set_phase("idle");
  ...
  size_t sz = f.size();
  Serial.printf("[FLASH] staged ATtiny image size=%u bytes\n", (unsigned)sz);
  if (sz == 0 || sz > ATTINY_FLASH_SIZE) {
    set_err("Bad ATTiny image size (max 4KB for ATtiny404)");
    f.close();
    g_last_ok = false;
    g_busy = false;
    set_phase("idle");
  ...
  if (f.read(buf, sz) != sz) {
    Serial.println("[FLASH] failed to read full staged ATtiny image");
  ...
  for (size_t i = 0; i < sz; i++) {
    checksum = (checksum * 33U) ^ buf[i];
  }
  Serial.printf("[FLASH] staged ATtiny image checksum=0x%08lX first=%02X %02X %02X %02X "
                "last=%02X %02X %02X %02X\n",
                (unsigned long)checksum, sz > 0 ? buf[0] : 0, sz > 1 ? buf[1] : 0,
                sz > 2 ? buf[2] : 0, sz > 3 ? buf[3] : 0, sz > 3 ? buf[sz - 4] : 0,
                sz > 2 ? buf[sz - 3] : 0, sz > 1 ? buf[sz - 2] : 0, sz > 0 ? buf[sz - 1] : 0);

  UPDIResult r = updi_program_flash(buf, sz, updi_progress_cb, nullptr);
  free(buf);

  if (r != UPDI_RESULT_OK) {
    Serial.printf("[FLASH] ATtiny flash failed: %s\n", updi_result_string(r));
    char detail[192];
    if (r == UPDI_RESULT_ENTER_PROG_FAILED) {
      snprintf(detail, sizeof(detail),
               "%s (ASI_System_Status=0x%02X masked=0x%02X; want 0x82 run or 0x08 prog; 0xFF=no UPDI reply)",
               updi_result_string(r), updi_last_asi_system_status(), updi_last_asi_system_status_masked());
    } else {
      snprintf(detail, sizeof(detail), "%s", updi_result_string(r));
    }
    set_err(detail);
    g_last_ok = false;
  } else {
    Serial.println("[FLASH] ATtiny flash complete");
    g_last_ok = true;
    set_err("");
  }
...
bool flash_jobs_status_json(char *buf, size_t bufLen) {
  ...
  int n = snprintf(buf, bufLen,
                     ...
                     "\"updi_echo_seen\":%s,\"updi_asi_status\":\"0x%02X\",\"updi_asi_status_masked\":\"0x%02X\","
                     "\"paths\":{\"eeprom\":\"%s\",\"attiny\":\"%s\"}}",
                     g_busy ? "true" : "false", g_progress, g_phase, g_last_ok ? "true" : "false",
                     errEsc, updi_last_echo_seen() ? "true" : "false",
                     updi_last_asi_system_status(), updi_last_asi_system_status_masked(),
                     PATH_EEPROM, PATH_ATTINY);
