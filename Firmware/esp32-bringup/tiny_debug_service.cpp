...
static uint16_t g_batStatus = 0;

// BQ76905 Battery Status (0x12) bits of interest.
static const uint16_t BATSTAT_SLEEP = (1u << 15);
static const uint16_t BATSTAT_DEEPSLEEP = (1u << 14);
static const uint16_t BATSTAT_FET_EN = (1u << 8);
static const uint16_t BATSTAT_POR = (1u << 7);
static const uint16_t BATSTAT_CFGUPDATE = (1u << 5);
...
static void parseStatus(const char *s) {
  // Format after 'S': "<v> <c1>,<c2> <b><p> <err> <batstatus>"
  // e.g. "S1 3701,3695 -P 0 388" (batstatus = BQ Battery Status 0x12, decimal)
  unsigned c1 = 0;
  unsigned c2 = 0;
  unsigned err = 0;
  unsigned bstat = 0;
  ...
  char bal = '-';
  char pwr = 'p';
  const int n = sscanf(s, "%c %u,%u %c%c %u %u", &v, &c1, &c2, &bal, &pwr, &err, &bstat);
  if (n < 6) {
  ...
  if (g_statusSeen) {
    ...
    if (n >= 7) {
      const uint16_t changed = (uint16_t)bstat ^ g_batStatus;
      if (changed & BATSTAT_CFGUPDATE) {
        pushEvent(bstat & BATSTAT_CFGUPDATE ? "BQ entered CFGUPDATE (ADC stopped!)"
                                            : "BQ left CFGUPDATE");
      }
      if (changed & (BATSTAT_SLEEP | BATSTAT_DEEPSLEEP)) {
        pushEvent(bstat & BATSTAT_DEEPSLEEP ? "BQ in DEEPSLEEP (no measurements!)"
                  : (bstat & BATSTAT_SLEEP) ? "BQ entered SLEEP mode"
                                            : "BQ in NORMAL mode");
      }
      if (changed & BATSTAT_POR) {
        pushEvent(bstat & BATSTAT_POR ? "BQ reports power-on reset" : "BQ POR flag cleared");
      }
    }
  } else {
...
static void handleLine(const char *l) {
  ...
  switch (l[0]) {
    ...
    case 'D': pushEvent("Power button down"); break;
    case 'L': pushEvent("Power button long press"); break;
    case 'R': pushEvent("Power button short press"); break;
