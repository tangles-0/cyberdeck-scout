...
static bool enable_bq_adc_once();
...
static bool enable_bq_adc_once() {
  uint8_t r2e = 0, r2f = 0, r30 = 0;
  if (!read_bq_reg(0x2E, r2e) || !read_bq_reg(0x2F, r2f) || !read_bq_reg(0x30, r30)) {
    return false;
  }
  r2e &= ~(0x7F);
  r2e |= 0x80;
...
static bool read_bq_via_tps(uint8_t *regs7, bool &cooldown) {
  cooldown = false;
  if (!tpsPresent) {
    return false;
  }
  ...
  uint8_t out[8] = {0};
  if (!read_tps_data1(out, 8)) {
    return false;
  }
  if (out[0] != 0) {
    return false;
  }
  memcpy(regs7, out + 1, 7);
  lastI2CrMs = now;
  return true;
}
...
bool charger_fill_json(char *buf, size_t bufLen) {
  ...
  bool have_bq_via = read_bq_via_tps(bqRegs, bqViaCooldown);

  uint8_t r1b = 0, r1c = 0, r20 = 0, r21 = 0;
  ...
  read_bq_reg16(0x3B, vbat);
  read_bq_reg16(0x3D, vsys);
  read_bq_reg16(0x35, vbus);
  ...
  int n = snprintf(
      ...
      "\"have_boot_flags\":%s,"
      "\"bq_direct_ok\":%s,\"reg1b\":%u,\"reg1c\":%u,\"fault0\":%u,\"fault1\":%u,"
      "\"vbat_mV\":%u,\"vsys_mV\":%u,\"vbus_mV\":%u,"
      "\"bq_via_tps_ok\":%s,\"bq_via_cooldown\":%s,"
      ...
      have_bq_dir ? "true" : "false", (unsigned)r1b, (unsigned)r1c, (unsigned)r20,
      (unsigned)r21, (unsigned)vbat, (unsigned)vsys, (unsigned)vbus,
      have_bq_via ? "true" : "false", bqViaCooldown ? "true" : "false",
      have_bq_via ? (unsigned)bqRegs[0] : 0, have_bq_via ? (unsigned)bqRegs[1] : 0,
